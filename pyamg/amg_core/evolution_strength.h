                         ...
                         const T2 tol)
{
    ...
    T2 * sing_vals = new T2[NullDimPone];
    ...
    const T2 near_zero = std::numeric_limits<T2>::epsilon();
    const T2 sqrt_near_zero = std::sqrt(near_zero);
    ...
    for(I i = 0; i < nrows; i++)
    {
        ...
        {
            T2 curr_norm = mynorm(zhat[zcounter]);
            if (curr_norm > max_zhat) {
                max_zhat = curr_norm; }
        }
        T2 tol_i = tol*max_zhat;
        for(I jj = rowstart, zcounter = 0; jj < rowend; jj++, zcounter++)
        {
        ...
        for(I jj = rowstart, zcounter = 0; jj < rowend; jj++, zcounter++)
        {
            ...
            else
            {
                ...
                const T2 dprod = real(zhat[zcounter])*real(z[zcounter]) + imag(zhat[zcounter])*imag(z[zcounter]);
                ...
                {
                    const T2 error = mynorm(-ratio + 1.0);
